...
void quickAbsorberPlot() 
{
  ...
  for(int i=0;i<=0000;i+=1000) {
    ...
    //    sprintf(inputName,"/unix/anita1/creamtea/strips_650/fakecontainer_10cmtargetat_1_3_1/pca/pca_fakecontainer_10cmtarget_%d_%d.root",i+1,i+1000);
     //    sprintf(inputName,"/unix/anita1/creamtea/strips_650/fakecontainer_10cmtargetat_0p5_3_0p5/pca/pca_fakecontainer_10cmtarget_%d_%d.root",i+1,i+1000);
     sprintf(inputName,"/unix/anita1/creamtea/strips_650/fakecontainer_30cmtargetat_0_0_0/pca/pca_fakecontainer_30cmtarget_%d_%d.root",i+1,i+1000);
     //     sprintf(inputName,"/unix/anita1/creamtea/strips_650/container_10cmtungsten/pca/pca_tungsten10cm_%d_%d.root",i+1,i+1000);
  ...
  can->Divide(2,2);
  can->cd(1);
  Absorbed->Draw("yGrad*0000+yCut:xGrad*0000+xCut","abs(xGrad*-7000+xCut)<5000 && abs(yGrad*-7000+yCut)<5000 &&xyzFitQual<0.5","colz");
  can->cd(2);
  Absorbed->Draw("yGrad*500+yCut:xGrad*500+xCut","abs(xGrad*-7000+xCut)<5000 && abs(yGrad*-7000+yCut)<5000&&xyzFitQual<0.5","colz");
  can->cd(3);
  Absorbed->Draw("yGrad*1000+yCut:xGrad*1000+xCut","abs(xGrad*-7000+xCut)<5000 && abs(yGrad*-7000+yCut)<5000&&xyzFitQual<0.5","colz");
  can->cd(4);
  Absorbed->Draw("yGrad*2000+yCut:xGrad*2000+xCut","abs(xGrad*-7000+xCut)<5000 && abs(yGrad*-7000+yCut)<5000&&xyzFitQual<0.5","colz");
